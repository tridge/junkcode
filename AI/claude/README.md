...
| [`reviewprs/`](reviewprs/) | `/reviewprs <label|@author>` command: review every open GitHub PR with a label (main + wiki + submodule repos) into one sortable HTML report with verdicts. Every PR gets both a Claude review and an independent Codex cross-check, fanned out one agent per PR; findings are posted back to the PRs as marked comments and the report published to the web (head-hash skip makes re-runs cheap). | `/reviewprs` |
