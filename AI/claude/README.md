...
| [`reviewprs/`](reviewprs/) | `/reviewprs <label>` command: review every open GitHub PR with a label (main + wiki + submodule repos) into one HTML report with verdicts, Codex-cross-checked and published to the web (with head-hash skip for fast re-runs). | `/reviewprs` |
...
files. The exception is `/reviewprs`, which queries GitHub (read-only via `gh`),
runs the `codex` CLI, and publishes its HTML report to the public ArduPilot web
host (`uav.tridgell.net/DevCallReviews/`).
