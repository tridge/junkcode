...
#define kmem_cache_create(name, size, i1, i2, ctor, p1) NULL
#define unlikely(x) x
#define likely(x) x
...
{
	while (offset<size && !test_bit(offset, addr)) offset++;
	return offset;
}
...
static int sub_remove(struct idr *idp, int shift, int id)
{
	...
	for (i=0;i<n;i++) {
		...
		if (present[ii]) {
			...
			if (random() % 5) {
				do {
					idr_pre_get(&idr, 0);
					ids[ii] = idr_get_new_above_int(&idr, &ids[ii], 100);
				} while (ids[ii] == -EAGAIN);
				if (ids[ii] < 0) {
